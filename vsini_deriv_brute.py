...
def plot_line_profile(line, obs_lambda, obs_flux, synth_data_fe, space = 4, np_rv=10, np_fit=20, vr = -1, plot_flag=True):
    ...
    dobs_flux = smooth(dobs_flux, 5)

    pobs = interpolate.interp1d(obs_lambda[ind_l], dobs_flux[ind_l], fill_value='extrapolate', bounds_error=False)
    psyn = interpolate.interp1d(obs_lambda[ind_l], dsyn_flux[ind_l], fill_value='extrapolate', bounds_error=False)
